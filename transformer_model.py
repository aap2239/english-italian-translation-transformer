...
class PositionalEncodings(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        pos_e = torch.zeros(self.seq_len, self.d_model)

        # Create a vector of shape (seq_len, 1)
        position_ = torch.arange(0, self.seq_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(
        ...
        pos_e[:, 0::2] = torch.sin(position_ * div_term)
        # Apply cosine on odd position
        pos_e[:, 1::2] = torch.cos(position_ * div_term)

        pos_e = pos_e.unsqueeze(0)

        self.register_buffer(
            "pos_e", pos_e
        )  # SAved in the file along with the state of the model

    def forward(self, x):
        x = x + self.pos_e[:, : x.shape[1], :].requires_grad_(False)
